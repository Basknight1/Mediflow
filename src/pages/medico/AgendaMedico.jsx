...
export default function AgendaMedico() {
      ...
      try {
        ...
        await Promise.all(
          citasConPago.map(async (cita) => {
            try {
              const pagoRes = await axios.get(`http://localhost:8083/pagos/cita/${cita.id}`);
  ...
  const abrirModalFinalizacion = async (cita) => {
    ...
    try {
      const res = await axios.get(`http://localhost:8082/citas/registros-consulta/cita/${cita.id}`);
              ...
              <div className="px-5 pb-5 sm:px-7 sm:pb-7">
                <div className="flex flex-col gap-5">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <textarea
                      className={`textarea textarea-bordered min-h-[110px] w-full resize-none ${
                        formErrors.observaciones ? "textarea-error" : ""
                      }`}
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <textarea
                      className={`textarea textarea-bordered min-h-[110px] w-full resize-none ${
                        formErrors.diagnostico ? "textarea-error" : ""
                      }`}
